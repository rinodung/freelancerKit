<div class="col-sm-12  col-md-12 main">  
	<div id="options" class="row">
		...
		<thead>
			...
			<th class="hidden-xs"><?=$this->lang->line('application_worker');?></th>
		...
		<tr id="<?=$value->id;?>" >
			...
			<td class="option" width="8%">
				        ...
				        <a href="<?=base_url()?>quotations/update/<?=$value->id;?>" class="btn-option" data-toggle="mainmodal"><i class="fa fa-cog"></i></a>
			</td>
		</tr>
		...
		<?php endforeach;?>
	 	...
	 	<div id="options" class="row">
			...
			<div class="btn-group pull-right-responsive margin-right-3">
		          ...
		          <ul class="dropdown-menu pull-right" role="menu">
		            <?php foreach ($submenu as $name=>$value):?>
			                <li><a id="<?php $val_id = explode("/", $value); if(!is_numeric(end($val_id))){echo end($val_id);}else{$num = count($val_id)-2; echo $val_id[$num];} ?>" href="<?=site_url($value);?>#custom_quotations_requests_filter"><?=$name?></a></li>
			            <?php endforeach;?>
		          </ul>
		      </div>
			<script type="text/javascript">$(document).ready(function() { 
		...
		<tr id="<?=$value->id;?>" >
			...
			<td class="option" width="8%">
				        ...
				        <a href="<?=base_url()?>quotations/cupdate/<?=$value->id;?>" class="btn-option" data-toggle="mainmodal"><i class="fa fa-cog"></i></a>
			</td>
		</tr>
